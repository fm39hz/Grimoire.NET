...
public sealed class XhtmlSegmentParser : IXhtmlSegmentParser {
	...
	private static void CollectFootnotes(IElement root, Dictionary<string, IElement> footnotes) {
		...
		foreach (var el in root.QuerySelectorAll("*[epub\\:type='footnote'], *[epub\\:type=footnote]")) {
			if (el.TagName.Equals("ASIDE", StringComparison.OrdinalIgnoreCase)) {
				continue;
			}
	...
	private void ProcessElement(
		...
		switch (tag) {
			...
			case "aside":
				if (el.GetAttribute("epub:type") == "footnote") {
					break;
				}
	...
	private static bool IsBlockTextDiv(IElement div) {
		...
		if (cls.Contains("text") || cls.Contains("content") || cls.Contains("body")) {
			return true;
		}

		return !div.HasChildNodes || div.ChildNodes.All(static c => c is IText || (c is IElement e && IsInlineElement(e)));
	}
	...
	private static string NormalizeImagePath(string src) => src.Replace('\\', '/').Split('/')[^1];
}
